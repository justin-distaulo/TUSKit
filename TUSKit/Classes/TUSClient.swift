...
public class TUSClient: NSObject, URLSessionTaskDelegate {
    ...
    public init(with config: TUSConfig) {
    ...
    public func createOrResume(forUpload upload: TUSUpload, withRetries retries: Int) {
        if (fileManager.fileExists(withName: upload.fileName) == false) {
            ...
            if let filePath = upload.filePath {
                do {
                    ...
                    self.delegate?.TUSFailure(forUpload: upload,
        ...
        if (status == .ready) {
            ...
            case .paused, .created:
                ...
                executor.upload(forUpload: upload, with: self)
            case .new:
                ...
                executor.create(forUpload: upload, with: self)
            default:
                break
            }
        }
